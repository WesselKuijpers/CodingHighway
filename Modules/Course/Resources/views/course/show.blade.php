              ...
              @else
                <div class="row">
                  <div class="col-sm-6 col-md-4 mb-2">
                    <a
                      href="{{ route('startExam.show', ['course_id' => $course->id, 'id' => $course->startExam->id]) }}"
                  ...
                  @foreach (Auth::user()->organisation()->groups->where('course_id', $course->id) as $group)
                    ...
                    <ul>
                      @foreach($group->students as $student)
                        <li>{{$student->getFullname()}} @if($student->results->where('course_id', $course->id)->count() != 0)
                            || {{$student->results->where('course_id', $course->id)->first()->level->name}} @endif</li>
                      @endforeach
                    </ul>
                    <hr>
                  @endforeach
