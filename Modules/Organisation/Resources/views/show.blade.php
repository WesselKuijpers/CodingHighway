            ...
            <div class="row">
              <div class="col-md-6 col-lg-6 col-sm-12">
                ...
                @if($organisation->users->count() == 0)
                  <p>Er zijn nog geen gebruikers die deel uitmaken van deze organisatie. Voeg gebruikers toe door
                    licentiecodes te genereren en uit te delen, na het aanmaken van een nieuw account zal gevraagd
                    worden om een licentiecode. Zodra een gebruiker een van de lcentiecodes invult die bij deze
                    organisatie hoort zal hij automatisch aan deze organisatie worden toegevoegd en zal je de voortgang
                    van deze gebruiker kunnen bijhouden.</p>
                @else
                  <ul class="list-group">
                    @foreach($organisation->users as $user)
                      @if($user->roles->where('slug', 'user')->count() == 1)
                        <li class="list-group-item">{{ $user->getFullname() }}
                          <button type="button" class="btn btn-primary btn-organisation float-right"
                                  data-toggle="collapse" data-target="#collapseExample{{ $user->id }}"
              ...
              <div class="col-md-6 col-lg-6 col-sm-12">
                ...
                <form action="{{ route('licenses.generate') }}" method="POST">
                  ...
                  @if (count($organisation->licenses) != 0)
                    ...
                    @endif
                  @else
                    <p>Geen licentiecodes gevonden, genereer licentiecodes zodat er gebruikers deel kunnen nemen aan
                      deze organisatie</p>
                  @endif
                </form>
              </div>
