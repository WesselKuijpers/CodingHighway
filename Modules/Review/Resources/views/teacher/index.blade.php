          ...
          @if($student->isUser())
            <li class="list-group-item">{{ $student->getFullname() }}
              <button type="button" class="btn btn-primary btn-organisation float-right" data-toggle="collapse"
                      data-target="#collapseStudent{{ $student->id }}" aria-expanded="false"
                ...
                <ul class="list-group">
                  @foreach($courses as $course)
                    <li class="list-group-item">{{ $course->name }}
                      ...
                      @endif
                      <button type="button" class="btn btn-primary btn-organisation float-right" data-toggle="collapse"
                              data-target="#collapseCourse{{ $course->id }}{{ $student->id }}" aria-expanded="false"
                              aria-controls="collapseExample">
                        <i class="fas fa-chevron-down"></i>
                      </button>
                    </li>
                          ...
                          <div class="row">
                            ...
                            <div class="col-3">
                              @if($exercise->solutions->where('user_id', $student->id)->count() == 1)
                                <a
                                  href="{{route('teacherCheckShow', ['id' => $exercise->solutions->where('user_id', $student->id)->first()->id])}}"
                                  class="btn btn-success">Kijk na</a>
                              @else
